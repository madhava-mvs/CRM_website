...
export default function Signup({ setshow, SignUp, error, error1, error2, error3, error4, error5, error6, Checkbox, setCheckbox, setEmail, setFirstname, setLastname, setPassword, setRepassword, firstname, lastname, email, password, repassword }) {
            ...
            <div className="signup_inner">
                ...
                <div className="signup_inner_row4">
                    <div className="signup_inner_row4_left">
                        ...
                        {error && firstname == "" ? <label className="signup_error">Firstname is mandatory</label> : ""}
                ...
                {error && email == "" ? <label className="signup_error"> email is mandatory</label> : ""}
                <label className="signup_err">{error6}</label>
                <div className="signup_inner_row6">
                    <input type="password" placeholder="Password" onChange={(e) => { setPassword(e.target.value) }} />
                </div>
                <label className="err">{error3}</label>
                {error && password == "" ? <label className="signup_error"> password  is mandatory</label> : ""}
                ...
                {error && repassword == "" ? <label className="signup_error"> Repassword is mandatory</label> : ""}
                <div className="signup_inner_row8">
                    <div className="signup_inner_row8_checkboxicon">
                        <input type="checkbox" onChange={(e) => { setCheckbox() }} />
                    </div>
                    <div className="signup_inner_row8_text">
                        <label>By clicking on Register, you are agreeing to our<br /> <span>Terms and Conditions</span> of Use.</label>
                    </div>
                    {error && Checkbox == false ? <label className="signup_Error"> Accept TnC</label> : ""}

                </div>

                <div className="signup_inner_row9">
                    <button onClick={(e) => { SignUp(e)/*setshow(true) */ }}> REGISTER</button>
                </div>
            </div>
        </div>
    </>);
